...
class AdminHomeController extends Controller
{
            ...
            if($email->user_password != $admin_password){
                ...
                if($email->user_login_fail >= 5){
                    ...
                    if($email->loainguoidung_id !=4){
                        ...
                        $permisson_admin = Permission::where('loainguoidung_id',$email->loainguoidung_id)->first();

                        Session::put('quyen_sanpham',$permisson_admin->quyen_sanpham);
                        Session::put('quyen_nhaphang',$permisson_admin->quyen_nhaphang);
                        Session::put('quyen_tintuc',$permisson_admin->quyen_tintuc);
                        Session::put('quyen_nhanvien',$permisson_admin->quyen_nhanvien);
                        Session::put('quyen_thongke',$permisson_admin->quyen_thongke);
                        Session::put('quyen_donhang',$permisson_admin->quyen_donhang);
                        Session::put('quyen_giaohang',$permisson_admin->quyen_giaohang);
    ...
    public function StaffAddSave(Request $request){
        ...
        if(Session::get('admin_role')!=1){
            ...
            $this->validate($request,[
                'staff_name' => 'bail|required|max:255|min:2',
                'staff_email' => 'bail|required|email|max:255',
                'staff_password' => 'bail|required|max:255|min:3',
                'staff_password_confirm' => 'bail|required|max:255|min:3'
            ],
            [
    ...
    public function StaffEditSave(Request $request, $staff_id){
        ...
        if(Session::get('admin_role')!=1){
            ...
            $data=$request->all();
            $this->validate($request,[
                'staff_first_name' => 'bail|required|max:255|min:2',
                'staff_last_name' => 'bail|required|max:255|min:2',
                'staff_phone_number' => 'bail|required|max:255|min:10',
                'staff_address' => 'bail|required|max:255|min:3',
                'staff_admin_id' => 'bail|required|max:255|min:3',
            ...
            if($get_admin){
                ...
                if ($get_image) {
                        ...
                        $get_image->move('uploads/admin/staff', $new_image);
    ...
    public function ShowStaffChangeEmail($staff_id){
        $this->AuthLogin();
        if(Session::get('admin_role')!=1){
            // $user_id=Session::get('admin_id');
            $user_staff=UserAccount::where('id',$staff_id)->first();
            return view('admin.pages.staff.staff_my_account_change_email')->with('user_staff',$user_staff);
        }else{
            $user_staff=UserAccount::where('id',$staff_id)->first();
            return view('admin.pages.staff.staff_my_account_change_email')->with('user_staff',$user_staff);
       }
    ...
    public function StaffChangeEmailSave(Request $request){
        ...
        if(Session::get('admin_role')!=1){
            ...
            $user_id=Session::get('admin_id');
            $user_staff=UserAccount::where('id',$user_id)->first();
            if (!$user_staff) {
    ...
    public function StaffChangePasswordSave(Request $request){
        ...
        $data=$request->all();
        $this->validate($request,[
            'my_account_old_password' => 'bail|required|max:255|min:3',
            'my_account_new_password' => 'bail|required|max:255|min:3',
            'my_account_confirm_new_password' => 'bail|required|max:255|min:3'
        ],
        [
            'required' => 'Không được để trống',
            'max' => 'Quá dài',
    ...
    public function StaffDelete($staff_id){
        $this->AuthLogin();
            ...
            $staff=Admin::where('id',$staff_id)->first('user_id');
            // return dd($staff);
            if (!$staff) {
                return Redirect::to('/staff')->with('error', 'Nhân viên không tồn tại!');
            } else {
               
                $get_import_product=ProductImport::where('admin_id', $staff_id)->first();
            
                if ($get_import_product) {
                    return Redirect::to('/staff')->with('error', 'Không thể xóa!');
                } else {
                    $delete_admin=admin::find($staff_id);
                    $delete_admin->delete();
                    $delete_user=UserAccount::find($staff['user_id']);
                    $delete_user->delete();

                    return Redirect::to('/staff')->with('message', 'Xóa thành công!');
                }
            }
    ...
    public function Role(){
        $this->AuthLogin();
        $role = DB::table('tbl_loainguoidung')->whereNotIn('tbl_loainguoidung.loainguoidung_loai',[4])->join('tbl_phanquyen','tbl_loainguoidung.loainguoidung_loai','=','tbl_phanquyen.loainguoidung_id')
        ->get();
        return view('admin.pages.staff.role')->with('role',$role);
    }
    public function RoleEdit(Request $request){
        $this->AuthLogin();
        $data = $request->all();
        $permission = Permission::where('loainguoidung_id',$data['role_id'])->first();
        if(isset($permission)){
            if($data['product_role']!=null)
            $permission->quyen_sanpham = $data['product_role'];
            if($data['import_product_role']!=null)
            $permission->quyen_nhaphang = $data['import_product_role'];
            if($data['news_role']!=null)
            $permission->quyen_tintuc = $data['news_role'];
            if($data['order_role']!=null)
            $permission->quyen_donhang = $data['order_role'];
            if($data['shipping_role']!=null)
            $permission->quyen_giaohang= $data['shipping_role'];
            if($data['staff_role']!=null)
            $permission->quyen_nhanvien= $data['staff_role'];
            if($data['statistics_role']!=null)
            $permission->quyen_thongke= $data['statistics_role'];
        $permission->save();
            ...
            $permission->quyen_nhanvien= $data['staff_role'];
            $permission->quyen_thongke= $data['statistics_role'];
            $permission->loainguoidung_id= $data['role_id'];
        }
        $permission->save();
        $role = DB::table('tbl_loainguoidung')->whereNotIn('tbl_loainguoidung.loainguoidung_loai',[4])->join('tbl_phanquyen','tbl_loainguoidung.loainguoidung_loai','=','tbl_phanquyen.loainguoidung_id')
        ->get();
        ...
        foreach($role as $key => $role_item){
            $role_id = $role_item->loainguoidung_id;
            $loainguoidung_ten= $role_item->loainguoidung_ten;
            if($role_item->quyen_sanpham==1){
                $quyen_sanpham = '<input class="form-check-input product_role product_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked> ';
            }
            else{
                $quyen_sanpham = '<input class="form-check-input product_role product_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="1" id="flexCheckChecked"> ';
            }
            if($role_item->quyen_nhaphang==1){
                $quyen_nhaphang = '<input class="form-check-input import_product_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_nhaphang= '<input class="form-check-input import_product_role_'.$role_id.'" type="checkbox" data-id_role="'.$role_id.'" value="1" id="flexCheckChecked">';
            }
            if($role_item->quyen_tintuc==1){
                
                $quyen_tintuc='<input class="form-check-input news_role news_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_tintuc='<input class="form-check-input news_role news_role_'.$role_id.'" type="checkbox" data-id_role="'.$role_id.'" value="1" id="flexCheckChecked">';
                
            }
            if($role_item->quyen_nhanvien==1){
                
                $quyen_nhanvien='<input class="form-check-input staff_role staff_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_nhanvien='<input class="form-check-input staff_role staff_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="1" id="flexCheckChecked"';
                
            }
            if($role_item->quyen_thongke==1){
                
                $quyen_thongke='<input class="form-check-input statistics_role statistics_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_thongke='<input class="form-check-input statistics_role statistics_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="1" id="flexCheckChecked"';
                
            }
            if($role_item->quyen_donhang==1){
                
                $quyen_donhang='<input class="form-check-input order_role order_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_donhang='<input class="form-check-input order_role order_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="1" id="flexCheckChecked"';
                
            }
            if($role_item->quyen_giaohang==1){
                
                $quyen_giaohang='<input class="form-check-input shipping_role shipping_role_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="0" id="flexCheckChecked" checked>';
            }else{
                $quyen_giaohang='<input class="form-check-input shipping_role staff_shipping_'.$role_id.'" data-id_role="'.$role_id.'" type="checkbox" value="1" id="flexCheckChecked"';
    ...
    public function RoleAddSave(Request $request){
        ...
        if(!isset($data['giaohang'])){
            $giaohang = 0;
        }else $giaohang = 1;
        if(!isset($data['thongke'])){
            $thongke = 0;
        }else $thongke = 1;
        $max_role= DB::table('tbl_loainguoidung')->max('loainguoidung_loai');
        $role = new  Role();
        $role->loainguoidung_loai= $max_role+1;
        $role->loainguoidung_ten=$data['role_name'];
        $role->save();
        $permission = new Permission();
        $permission->loainguoidung_id = $max_role+1;
        $permission->quyen_sanpham = $sanpham;
        $permission->quyen_tintuc=$tintuc;
        $permission->quyen_nhaphang=$nhaphang;
        $permission->quyen_nhanvien=$nhanvien;
        $permission->quyen_giaohang=$giaohang;
        $permission->quyen_donhang=$donhang;
        $permission->quyen_thongke=$thongke;
        $permission->save();
    ...
    public function RoleDelete(Request $request){
        ...
        $data=$request->all();
        $role=Role::where('loainguoidung_loai',$data['role_id'])->first();
        $role->delete();
        $permission=Permission::where('loainguoidung_id',$data['role_id'])->first();
        $permission->delete();
    }
