            ...
            $search_keyword=$data['query'];
                ...
                if($product_id!=null){
                    if ($discount->count()>0) {
                        ...
                        $all_product=Product::whereIn('id', $product_id)->where('sanpham_trang_thai', '1')->whereNotIn('id', $pro_dis)
                        ->where('sanpham_ten', 'like', '%'.$search_keyword.'%')->orderBy('id', 'DESC')->get();
                        // $all_product_viewed=Product::whereIn('id', $view_id)->where('sanpham_trang_thai', '1')
                        // ->whereIn('id', $product_id)
                        // ->whereNotIn('id', $pro_dis)->orderBy('id', 'DESC')->get();
                    }else{
                ...
                $output= '<ul class="dropdown-menu" style="display:block;position:relarive">';
                foreach($all_product as $key =>$val){
                    $output .='<li class="hover-autocomplete"><a href="'.URL::to('/product-detail/'.$val->id).'" >
                    <table>
                    <tr>
                    <td>
                    <img src="'.asset('/uploads/admin/product/'.$val->sanpham_anh).'" width="50px"">
                    </td>
                    <td>
                    '.$val->sanpham_ten.'
                    <p style="color:red">'.number_format( $val->sanpham_gia_ban,0,',','.' )." VND".'</p>
                    </td>
                    </tr>
                    </table>
    ...
    public function ShowProductSearchFilterCustomer(Request $request){
        ...
        if (!$search_filter  || $search_filter==null) {
            ...
            if ($search_customer_size!=null ) {
                $in_stock=ProductInStock::where('sanphamtonkho_so_luong_ton', '>', 0)->where('size_id',$search_customer_size)->orderby('id', 'DESC')->get();
                if(!$in_stock){
