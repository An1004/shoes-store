...
class ProductController extends Controller
{
    ...
    public function ProductSave(Request $request){
        ...
        if(Session::get('admin_role')==3){
            ...
            //     'product_description' => 'bail|required|min:3',
            //     'product_color' => 'required',
            //     'product_img' => 'bail|mimes:jpeg,jpg,png,gif|max:10000',
            //     'product_feature' => 'bail|required|max:255|min:3',
            //     'product_production' => 'bail|required|min:3',
            //     'product_accessories' => 'bail|required|max:255|min:3',
            //     'product_material' => 'bail|required|max:255|min:3',
            //     'product_guarantee' => 'bail|required|max:255|min:3'
            // ],
            // [
            ...
            if ($get_product) {
                ...
                if ($get_image) {
                    ...
                    $get_image = $request->file('product_img');
                    $path = 'uploads/admin/product';
                    //them hinh anh
                    if ($get_image) {
                        if($product->sanphamanh==$get_image){
                            ...
                            $get_image->move('uploads/admin/product', $new_image);
            ...
            if (!$product) {
                ...
                if ($get_image) {
                    foreach ($get_image as $image) {
                        ...
                        $image->move('uploads/admin/productimages', $new_image);
    ...
    public function ProductPriceSize($product_id){
        $this->AuthLogin();
        if(Session::get('admin_role')==3){
            return Redirect::to('/dashboard');
        }else{
            $product = Product::find($product_id);
            $price = DB::table('tbl_gia')->where('tbl_gia.sanpham_id',$product_id)->join('tbl_size','tbl_size.id','=','tbl_gia.size_id')
            ->select('tbl_gia.id as id','tbl_gia.size_id as size_id','gia_ban','size_thu_tu','size','tbl_gia.sanpham_id as sanpham_id')->get();
            $get_size = Size::all();

                    return view('admin.pages.products.product_price')
                    ->with('product',$product)
    ...
    public function PriceSizeSave(Request $request,$product_id){
        $this->AuthLogin();
        $data=$request->all();
        $get_size=Price::where('sanpham_id',$product_id)->where('size_id',$data['get_size'])->get();
        if(count($get_size)==0){
            $price = new Price();
            $price->gia_ban=$data['price'];
            $price->size_id=$data['get_size'];
            $price->sanpham_id=$product_id;
            $price->save();
            return Redirect::to('/product-price-size/'.$product_id)->with('massage','Thêm thành công!');
            
        }
        else return Redirect::to('/product-price-size/'.$product_id)->with('error', 'Thêm không thành công, size đã tồn tại!');


    }
    public function ProductPriceSizeSave(Request $request, $product_id){
        $this->AuthLogin();
        if(Session::get('admin_role')==3){
            return Redirect::to('/dashboard');
        }else{
            ...
            $price = Price::where('sanpham_id',$product_id)->get()->where('size_id',$data['size_id'])->first();
            $price->gia_ban = $data['price'];
            $price->save();
    ...
    public function DeletetPriceSize(Request $request, $price_id){
        $this->AuthLogin();
        if(Session::get('admin_role')==3){
            return Redirect::to('/dashboard');
        }else{
            $get_price=Price::where('id',$price_id)->first();
            $get_import=ProductImportDetail::where('sanpham_id',$get_price->sanpham_id)->where('size_id', $get_price->size_id)->first();
            $get_in_stock=ProductInStock::where('sanpham_id',$get_price->sanpham_id)->where('size_id', $get_price->size_id)->first();
            $get_order=OrderDetail::where('sanpham_id',$get_price->sanpham_id)->where('size_id', $get_price->size_id)->first();
            if ($get_order || $get_in_stock || $get_import) {
                return Redirect::to('/product-price-size/'.$get_price->sanpham_id)->with('error', 'Không thể xóa!');
            } else {
                ...
                if (!$get_price) {
                    ...
                    $get_price->delete();
                    return Redirect::to('/product-price-size/'.$get_price->sanpham_id)->with('message', 'Xóa thành công!');
                }
            }
        }
    }
