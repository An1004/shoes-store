...
class StatisticsController extends Controller
{
    ...
    public function ShowProductViewsStatistics(){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $all_product_views=ProductViews::orderby('viewssanpham_ngay_xem', 'DESC')->paginate(20);
        ...
        if (Session::get('admin_role')==3) {
            ...
            <td>
                <a href="javascript: void(0);">
                    <img src="'.asset('/uploads/admin/product/'.$views->Product->sanpham_anh).'" alt="contact-img" title="contact-img" class="avatar-lg rounded-circle img-thumbnail">
                </a>
            </td>
            <td>
    ...
    public function ShowProductInStockStatistics(){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $all_product_in_stock_statistics=ProductInStock::orderby('id', 'DESC')->paginate(20);
            $all_size=Size::orderby('size_thu_tu', 'ASC')->get();
            $all_order_success=DB::table('tbl_dondathang')->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
            ->where('dondathang_trang_thai',3)
            ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
            foreach($all_order_success as $key => $all){
                $all = $all;
                $money[] = $all->total;
                $sl[] = $all->sluong;
                $tdon[] = $all->tdon;
                $day[] = date('d-m-Y', strtotime( $all->dondathang_ngay_dat_hang));
            }
    ...
    public function SearchProductInStockStatistics(Request $request){
        ...
        if (Session::get('admin_role')==3) {
            ...
            <tr>
                <td>
                    <a href="javascript: void(0);">
                        <img src="'.asset('/uploads/admin/product/'.$product_in_stock->Product->sanpham_anh).'" alt="contact-img" title="contact-img" class="avatar-lg rounded-circle img-thumbnail">
                    </a>
                </td>
                <td>
    ...
    public function ShowImportStatistics(){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $all_product_import_statistics=ProductImport::orderby('id', 'DESC')->paginate(20);
    ...
    public function ShowSalesStatistics(){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $all_order_statistics=Order::orderby('id', 'DESC')->paginate(6);
    ...
    public function SearchOrderStatistics(Request $request){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '>=', $data['from_day'])
            ->whereDate('dondathang_ngay_dat_hang', '<=', $data['to_day'])->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
               ->where('dondathang_trang_thai','=',3)
               ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
               foreach($all_order_success as $key => $all){
                $datas[] = array(
            ...
            ->whereDate('dondathang_ngay_dat_hang', '<=', $data['to_day'])->sum('dondathang_tong_tien');
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '<=', $data['to_day'])->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                foreach($all_order_success as $key => $all){
                    $datas[] = array(
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '>=', $data['from_day'])->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                foreach($all_order_success as $key => $all){
                    $datas[] = array(
            ...
            if ($id_order !=null) {
                ...
                // $all_order_success=DB::table('tbl_dondathang')->whereIn('id', $id_order)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                // ->where('dondathang_trang_thai','=',3)
                // ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                // foreach($all_order_success as $key => $all){
                //     $all = $all;
            ...
            <table class="table table-hover m-0 table-centered dt-responsive nowrap w-100 " cellspacing="0" id="tickets-table">
                ...
                <tr >
                    ...
                    <tbody class="font-14 " >
                        ...
                        <tr style="color:forestgreen; font-weight:bold; background-color:rgb(231, 245, 240);">
                            <td>
                ...
                <table class="table table-hover m-0 table-centered dt-responsive nowrap w-100 " cellspacing="0" id="tickets-table">
                    <thead class="bg-light">
                    <tr style="background-color: #F8C4B4; color:black; font-weight:bold">
                    ...
                    <tbody class="font-14 " >
                        ...
                        <tr style="background-color: #f8f5f5; color:#FF8787; font-weight:bold; text-align:center">
                            <td>
                ...
                <label class="col-form-label"> <h4>Các Đơn Hàng Gần Đây</h4></label>
                <table class="table table-hover m-0 table-centered dt-responsive nowrap w-100 " cellspacing="0" id="tickets-table">
                <thead class="bg-light">
                <tr style="background-color: #fff59f; color:black; font-weight:bold">
            ...
            <tbody class="font-14 show_views_type_search" >';
            foreach ($all_order_statistics as $key=>$order) {
                $output .= ' <tr style="background-color: #faffd5; color:#43b103; font-weight:bold">
                        <td>
    ...
    public function SearchOrderStatisticsChart(Request $request){
        $this->AuthLogin();
        if (Session::get('admin_role')==3) {
            return Redirect::to('/dashboard');
        } else {
    ...
    public function SearchSelectOrderStatisticsChart(Request $request){
        $this->AuthLogin();
        if (Session::get('admin_role')==3) {
            return Redirect::to('/dashboard');
        } else {
            ...
            $date_year=date("Y-m-d", strtotime($date_now . "- 365  day"));
            if ($data['search_type']==1) {
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '=', $date_now)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
            ...
            return json_encode($datas);
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '<=', $date_now)->whereDate('dondathang_ngay_dat_hang', '>=', $date_week)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                if(count($all_order_success) == 0){
                    $datas[]= array("sl" => 0,
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '<=', $date_now)->whereDate('dondathang_ngay_dat_hang', '>=', $date_month)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                if(count($all_order_success) == 0){
                    $datas[]= array("sl" => 0,
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '<=', $date_now)->whereDate('dondathang_ngay_dat_hang', '>=', $date_quarter)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                if(count($all_order_success) == 0){
                    $datas[]= array("sl" => 0,
                ...
                $all_order_success=DB::table('tbl_dondathang')->whereDate('dondathang_ngay_dat_hang', '<=', $date_now)->whereDate('dondathang_ngay_dat_hang', '>=', $date_year)->join('tbl_chitietdondathang','tbl_chitietdondathang.dondathang_id','=','tbl_dondathang.id')->select('dondathang_ngay_dat_hang',DB::raw('SUM(chitietdondathang_so_luong) as sluong'),DB::raw('COUNT(dondathang_ma_don_dat_hang) as tdon') , DB::raw('SUM(dondathang_tong_tien) as total'))
                ->where('dondathang_trang_thai','=',3)
                ->groupBy('dondathang_ngay_dat_hang')->orderby('dondathang_ngay_dat_hang', 'ASC')->get();
                if(count($all_order_success) == 0){
                    $datas[]= array("sl" => 0,
