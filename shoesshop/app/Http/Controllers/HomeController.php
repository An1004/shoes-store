...
class HomeController extends Controller
{
    ...
    public function Index(){
        ...
        $all_blog = BlogNews::where('baiviet_trang_thai','1')->orderBy('id','DESC')->get();
    ...
    //                         <a href="single-product.html"><img id="wishlist_viewed_product_image'.$pro_wishlist->id.'" src="'.asset('/uploads/admin/product/'.$pro_wishlist->sanpham_anh).'" alt=""></a>
    //                         <div class="hot_img">
    ...
    public function ProductDetail($product_id){
        ...
        Session::save();
        $comment_customer=Comment::where('sanpham_id',$product_id)->where('binhluan_id_phan_hoi','=',0)->where('binhluan_trang_thai',1)->get();
        $comment_customer= DB::table('tbl_binhluan')->join('tbl_khachhang','tbl_binhluan.khachhang_id','=','tbl_khachhang.id')->where('sanpham_id',$product_id)->where('binhluan_id_phan_hoi','=',0)->where('binhluan_trang_thai',1)->get();
        ...
        $all_size=ProductInStock::select('size_id')->where('sanpham_id',$product_id)->where('sanphamtonkho_so_luong_ton','>',0)->orderBy('id','asc')->get();
        $get_in_stock=ProductInStock::where('sanpham_id',$product_id)->get();
        $product_in_stock=ProductInStock::where('sanpham_id',$product_id)->orderBy('size_id','asc')->first();
        $all_product_type=ProductType::orderby('id','desc')->get();
        $all_brand=Brand::orderby('id','desc')->get();
        $all_collection=Collection::orderby('id','desc')->get();
        $all_header=HeaderShow::where('headerquangcao_trang_thai','1')
        ->orderby('headerquangcao_thu_tu','ASC')->get();
        $get_price = DB::table('tbl_sanphamtonkho')->get();
        if($all_header->count()>0){
            foreach($all_header as $key=>$value){
                $thu_tu_header=$value->headerquangcao_thu_tu;
                break;
        ...
        $get_about_us_bottom=AboutStore::orderby('cuahang_thu_tu','ASC')->first();
        $test_order=DB::table('tbl_chitietdondathang')->join('tbl_dondathang','tbl_dondathang.id','=','tbl_chitietdondathang.dondathang_id')->where('tbl_dondathang.dondathang_trang_thai','=',3)->where('sanpham_id',$product_id)->get();
        // return dd($test_order);
    	return view('client.pages.products.product_detail')
        ->with('product',$get_product)
        ->with('all_size',$all_size)
        ->with('get_in_stock',$get_in_stock)
        ->with('product_in_stock',$product_in_stock)
        ->with('test_order',$test_order)
        ->with('all_product',$all_product)
        ->with('all_product_viewed',$all_product_viewed)
        ->with('product_type',$all_product_type)
        ->with('product_brand',$all_brand)
    ...
    public function GetProductImage(Request $request){
        $data = $request->all();
        $find_img = ProductImage::where('id',$data['id'])->first();
        $output1 = '';
        $output1 = '  
             <img id="wishlist_viewed_product_image'.$find_img->sanpham_id.'" src="'.asset('/uploads/admin/productimages/'.$find_img->anhsanpham_ten).'" width="380px" height="400px" alt="">
    ...
    public function ChangePrice(Request $request){
        $data = $request->all();
        $find_price = Price::where('sanpham_id',$data['product_id'])->where('size_id',$data['size_id'])->first();
        $output1 = '';
        $output1 = '<input type="hidden" name="product_price" class="product_price_'.$find_price->sanpham_id.'" value="'.number_format($find_price->gia_ban ,0,',','').'" >';

       echo $output1;

    }
    public function ChangePriceHidden(Request $request){
        $data = $request->all();
        $find_price = Price::where('sanpham_id',$data['product_id'])->where('size_id',$data['size_id'])->first();
        $output = '';
        $output .= '<span>
        '.number_format($find_price->gia_ban ,0,',','.').'';
        $output .=' VNĐ </span>';
       echo $output;
    }
    public function ChangeProductInStock(Request $request){
        $data = $request->all();
        $find_product = ProductInStock::where('sanpham_id',$data['product_id'])->where('size_id',$data['size_id'])->first();
        $output = '';
        if($find_product->sanphamtonkho_so_luong_ton > 0)
            $item= 'Trong kho';
            else
            $item= 'Bán hết';

        $output .= ' <p>'.$find_product->sanphamtonkho_so_luong_ton.' sản phẩm</p>
        <span>
    ...
    public function ShowOrderTracking(){
        ...
        if(!$data || !$time_order_tracking || $time_order_tracking < $now){
            ...
            return Redirect::to('/order-tracking')->with('error','Vui lòng nhập mã đơn hàng hoặc email đăng ký để kiểm tra đơn hàng!');
        }else{
            ...
            if(!$order && $order_delivery->count()<=0 && !$order_user){
                return Redirect::to('/order-tracking')->with('error','Không tìm thấy!');
            }elseif($order){
    ...
    public function ShowBlog($blog_id){
        $blog= BlogNews::where('id',$blog_id)->first();
        ...
        $get_about_us_bottom=AboutStore::orderby('cuahang_thu_tu', 'ASC')->first();
        // return dd($blog);
        return view('client.pages.news.blog_new')->with('blog',$blog) ->with('product_type', $all_product_type)
        ->with('product_brand', $all_brand)
    ...
    public function Blog(){
        $all_blog = BlogNews::where('baiviet_trang_thai','1')->orderBy('id','DESC')->paginate(4)->fragment('all_blog');
        $all_about_us=AboutStore::orderby('cuahang_thu_tu', 'ASC')->get();
        $all_product_type=ProductType::where('loaisanpham_trang_thai', '1')->orderBy('id', 'DESC')->get();
        $all_brand=Brand::where('thuonghieu_trang_thai', '1')->orderBy('id', 'DESC')->get();
        $all_collection=Collection::where('dongsanpham_trang_thai', '1')->orderBy('id', 'DESC')->get();
        ...
        $get_about_us_bottom=AboutStore::orderby('cuahang_thu_tu', 'ASC')->first();
        // return dd($blog);
        return view('client.pages.news.show_new')->with('all_blog',$all_blog) ->with('product_type', $all_product_type)
        ->with('product_brand', $all_brand)
        ->with('all_about_us', $all_about_us)
        ->with('get_about_us_bottom', $get_about_us_bottom)
        ->with('product_collection', $all_collection)
