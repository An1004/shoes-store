...
class ProductImportController extends Controller {
	...
	public function ProductImportAddQueue(Request $request) {
        ...
        if (Session::get('admin_role')==3) {
            ...
            $size=DB::table('tbl_gia')->where('sanpham_id',$data['product_id'])->join('tbl_size','tbl_size.id','=','tbl_gia.size_id')
            ->select('tbl_size.id as id','tbl_size.size','tbl_size.size_thu_tu')->get();
            foreach ($size as $key => $item)
            {
                $get_size[] = array(
                    'id'=>$item->id,
                    'size'=>$item->size,
                    'size_thu_tu'=>$item->size_thu_tu
	...
	public function ProductImportAddMultipleSave(Request $request) {
        ...
        if(Session::get('admin_role')==3){
            ...
            $this->validate($request,[
                'product_import_no' => 'bail|required|max:255|min:3',
            ],
            [
    ...
    public function ProductImportAddSave(Request $request){
        ...
        if(Session::get('admin_role')==3){
            ...
            return Redirect::to('/product-import-add')->with('message', 'Thêm thành công');
        }
    }
    ...
    public function ProductImportShowDetail($product_import_id){
        ...
        if(Session::get('admin_role')==3){
            ...
            if (!$product_import) {
                ...
                ;
                // return dd($product_stock->sanphamtonkho_so_luong_ton);
	...
	public function ProductImportEditDetail($product_import_detail_id) {
        ...
        if(Session::get('admin_role')==3){
            ...
            if (!$product_import_detail) {
                ...
                $all_size=DB::table('tbl_gia')->where('sanpham_id',$product_import_detail->sanpham_id)->join('tbl_size','tbl_size.id','=','tbl_gia.size_id')
                ->select('tbl_size.id as id','tbl_size.size','tbl_size.size_thu_tu')->orderBy('id', 'DESC')->get();
	...
	public function ProductImportEditDetailSave(Request $request, $product_import_detail_id) {
        ...
        if (Session::get('admin_role')==3) {
            ...
            if (($import_product_in_stock_old->sanphamtonkho_so_luong_ton - $data['product_import_detail_quantity_old']) < 0) {
                return Redirect::to('/product-import-show-detail/'.$import_product_detail->donnhaphang_id)->with('message', 'Cập nhật không thành công');
    ...
    public function SelectImageProduct(Request $request){
        ...
        if (Session::get('admin_role')==3) {
            ...
            $output = '<img class=" mt-3" width="300px" height="370px" id="image" src="'.asset('/uploads/admin/product/'.$product->sanpham_anh).'" />';
            echo  $output;
        }
    }
}